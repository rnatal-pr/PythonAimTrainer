...
class Target:
    ...
    def draw(self, win): #order of drawing is importance
...
def end_screen(win, elapsed_time, target_pressed, clicks):
    ...
    accuracy_label = LABEL_FONT.render(f"Accuracy: {target_pressed}%", 1, "white")
...
def main():
    ...
    while run:
        ...
        for target in targets:
            ...
            if target.size <= 0: #remove if target shrinks
                targets.remove(target)
                misses += 1
                                     #(mouse_pos[0], mouse_pos[1])
            if click and target.collide(*mouse_pos): # "*" breaks down tuple into individual components
                targets.remove(target)
                target_pressed += 1
